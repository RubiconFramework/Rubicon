...
using Rubicon.Autoload.Resources;
using Rubicon.Data;
...
public partial class VolumeManager : CanvasLayer
{
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if (masterVolumeLabel.Visible)
            masterVolumeBar.Value = Mathf.Lerp((float)masterVolumeBar.Value, SaveData.Audio.MasterVolume, (float)delta * 10);

        if (Input.IsActionJustPressed("volume_up") && !isVolumePanelShown)
        {
            float newVolume = Mathf.Clamp(SaveData.Audio.MasterVolume + 10, 0, 100);
